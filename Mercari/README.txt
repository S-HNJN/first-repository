...
텍스트를 벡터로 표현하는 방법을 찾다가 **TF-IDF**를 알아냈고, 이에 대해 공부해보았다.
...
- **113개 중분류 중 101개에서, “brand_name”가 1순위 Feature**  
- **5개 중분류에서는 “cate3(소분류)”가 1순위 Feature**  
- **각 중분류별 서로 다른 긍부정(가격과의 영향력) 단어들**
- **상위 긍정 단어들 내에 브랜드 및 카테고리 이름이 다수 포함**

이는 두 가지를 동시에 시사한다.

1. **브랜드 혹은 소분류 정보만으로도 가격의 상당 부분을 설명할 수 있는 중분류가 대다수**라는 점  
2. “제품 설명 길이” 수준의 단순 피처는, 단독으로 1순위가 될 정도의 설명력을 갖고 있지 않다는 점
...
이 질문이, 텍스트 분석(TF-IDF)과 스태킹을 도입하게 된 동기가 되었다.
...
- 텍스트(`name`, `item_description`)에서 브랜드명 및 카테고리명에 해당하는 문자열을 제거  
...
- CatBoost (고려했으나 너무 오래 걸림)
- RandomForest (고려했으나 너무 오래 걸림)
...
- **전체 모델이 중분류별로 나눈 초기 모델(TF-IDF + Ridge + LightGBM)보다 성능이 더 좋았다.**  
